...
$str=$_GET["str"];
                if ($by == "good") {
                        ...
                        $all_description = $db->prepare("SELECT *FROM `description` as d LEFT JOIN course_comment  as c ON d.dId=c.post_id WHERE course_name LIKE 
                        CONCAT('%', :course_name, '%') GROUP BY d.dId ORDER BY COUNT(d.dId) DESC ");
                }else if($by=="def"){
                        $all_description = $db->prepare("SELECT *FROM`description` WHERE course_name LIKE 
                        CONCAT('%', :course_name, '%') ORDER BY `upload_time`DESC");
                }else if($by=="like"){
                        $all_description = $db->prepare("SELECT *FROM`description` WHERE course_name LIKE 
                        CONCAT('%', :course_name, '%') ORDER BY like_num DESC");
                }
                ...
                if ($resultCount > 0) {
                    while ($row = $all_description->fetch()) { 
                        $us=$row->dId;
