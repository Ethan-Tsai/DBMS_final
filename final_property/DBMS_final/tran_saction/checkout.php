    ...
    <div class="paypal">
      ...
      <div class="paypal__header">
        ...
        <div class="paypal__header-info">
          <span class="paypal__date">        <script language="javascript">
　var Today=new Date();
　document.write(Today.getFullYear()+ " 年 " + (Today.getMonth()+1) + " 月 " + Today.getDate() + " 日");
</script>
</span>
          <span class="paypal__ref">
          <script language="javascript">
　var Today=new Date();
　document.write(Today.getHours()+ " 時 " + (Today.getMinutes()+1) + " 分 " + Today.getSeconds() + "秒");
</script>
          </span>
        </div>
      </div>
...
$sql = $db->query("SELECT * FROM `member` WHERE `mId` =$user");
foreach($sql as $row){
  $user_name=$row["name"];
}
?>
      <div class="paypal__subheader-wrapper">
        <div class="paypal__subheader">
          <h1 class="paypal__username"><?=$user_name?></h1>
...
?>
          ...
          <li class="paypal__cart-item">
            ...
            <span class="paypal__item-name"><?=$row["bookname"]?></span>
            <span class="paypal__item-price"><?=$row["price"]?></span>
          </li>
<?php
        }
...
?>
          ...
          <?php
                ...
                $sql = $db->query("SELECT book.bookname,book.mId,price FROM `cart_new` INNER JOIN `book` ON book.mid = cart_new.book_mid AND book.bookname=cart_new.bookname WHERE `user_id` = $user_id;");
                $price = $db->query("SELECT SUM(price) FROM `cart_new` INNER JOIN `book` ON book.mid = cart_new.book_mid AND book.bookname=cart_new.bookname WHERE `user_id` = $user_id;");
        ...
        ?>
                  <li class="paypal__cart-item">
                    <span class="paypal__cart-total">Total</span>
                    <span class="paypal__item-price"><?=$price_final?></span>
                  </li>
...
<input type="hidden" name="price" value="<?=$price_final?>">
    ...
    <span class="inspired-by">
      <!-- <a href="https://dribbble.com/shots/2738907-PayPal-Email-Receipt">吱吱庫庫</a> -->
    </span>
